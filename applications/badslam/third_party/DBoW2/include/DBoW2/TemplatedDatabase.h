...
public:
  ...
  void remove(int entry_id, const BowVector &v);
...
template<class TDescriptor, class F>
void TemplatedDatabase<TDescriptor, F>::remove(int entry_id, const BowVector &v) {
  if(m_use_di)
  {
    // update direct file
    m_dfile[entry_id] = DBoW2::FeatureVector();
  }
  ...
  for(vit = v.begin(); vit != v.end(); ++vit)
  {
    ...
    IFRow& ifrow = m_ifile[word_id];
    // NOTE: Binary search might be better here in principle, but IFRow is a
    //       std::list, so it is not applicable.
    typename IFRow::iterator iit;
    for(iit = ifrow.begin(); iit != ifrow.end(); ++ iit) {
      if (iit->entry_id == entry_id) {
        ifrow.erase(iit);
        // There can only be one entry for each image.
        break;
      }
    }
  }
  ...
  QueryResults &ret, int max_results, int max_id) const
{
  BowVector::const_iterator vit;
  typename IFRow::const_iterator rit;
  double logeps = log(DBL_EPSILON); // XXX HACK
